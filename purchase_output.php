...
<body>
	...
	<?php
		...
		$purchase_id = 1;
		foreach ($pdo->query('select maax(id) from purchase')as $row) {
		...
		$stm->bindValue(':customer_id', $_SESSION['customer']['ud'], PDO::PARAM_INT);
		if ($stm->execute()) {
			foreach ($_SESSION['product'] as $product_id => $product) {
				$sql = "insert into purchase values(:purchase_id,:product_id,:count)";
				$stm = $pdo->prepare($sql);
